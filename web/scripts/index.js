...
const spritesheetScript = path.join(
  __dirname,
  "create-separate-spritesheets.js",
);
...
if (helpRequested) {
  ...
  console.log(
    "  --skip-sprite, -s  スプライトシート生成をスキップ（画像抽出のみ）",
  );
  console.log("  --help, -h         このヘルプメッセージを表示");
  console.log("\n例:");
  console.log("  node scripts/index.js                  # 基本実行");
  console.log(
    "  node scripts/index.js --cleanup        # クリーンアップ付き実行",
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        reject(
          new Error(
            `${path.basename(scriptPath)} がエラーコード ${code} で終了しました`,
          ),
        );
      } else {
        resolve();
...
function checkBackupFile() {
  ...
  if (!fs.existsSync(backupPath)) {
    ...
    console.log("\n以下のいずれかの方法で準備してください:");
    console.log(
      "1. icons.ts をバックアップ: cp src/app/bg/icons.ts src/app/bg/icons.backup.ts",
    );
