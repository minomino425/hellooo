...
iconsArray.forEach((icon, index) => {
  try {
    ...
    // メイン画像データを処理
    if (icon.data) {
      const base64Match = icon.data.match(/^data:image\/(jpeg|jpg|png|webp);base64,(.+)$/);
      if (base64Match) {
    ...
    // QRコード画像データを処理
    if (icon.qr) {
      const qrMatch = icon.qr.match(/^data:image\/(jpeg|jpg|png|webp);base64,(.+)$/);
      if (qrMatch) {
    ...
    console.error(`✗ エラー: アイコン ${index} (${icon.account || 'unknown'}):`, error.message);
    errorCount++;
  }
});
...
console.log('2. icons-new.tsをicons.tsにリネーム');
console.log('3. 画像読み込み処理を更新（Assets.loadを使用）');
console.log('========================================');