...
async function createSpritesheet(imageDir, outputName, type) {
  ...
  if (files.length === 0) {
    const webpFiles = fs.readdirSync(imageDir)
...
async function main() {
  ...
  try {
    ...
    const iconData = await createSpritesheet(iconDir, 'icons-sprite.png', 'アイコン');
    ...
    console.log(`読み込み時間: 大幅に短縮`);
    console.log('\n次のステップ:');
    console.log('1. 生成されたスプライトシートを確認');
    console.log('2. index.tsを更新してスプライトシートを使用');
    console.log('3. 個別画像ファイルを削除（オプション）');
    console.log('========================================');
